...
int main()
{
    ...
    uint16_t rxdiff_cipher[2];//the RX-difference of cipher

    uint16_t tmp_rxdiff_cipher[2];
    ...
    uint16_t rxdiff_cipher_lft[13][16];
    uint16_t rxdiff_cipher_rgt[13][16];
    ...
    uint16_t rxdiff_guess_lft_bit[3][16];
    uint16_t rxdiff_guess_rgt_bit[3][16];


    uint16_t rxdiff_guess_lft[3] = {0};
    uint16_t rxdiff_guess_rgt[3] = {0};
    ...
    for(int i = 0 ; i < 32768; i++)//Taking 2^{15} pairs of plaintexts with the input RX-difference (0020,0464)
    {
        ...
        rxdiff_cipher[1] = ROL(cipher[1], 1) ^ cipher_pie[1];
        rxdiff_cipher[0] = ROL(cipher[0], 1) ^ cipher_pie[0];


        tmp_rxdiff_cipher[1] = rxdiff_cipher[1] & 0xe210;//delta-lft=000* **0* ***1 ****

        tmp_rxdiff_cipher[0] = rxdiff_cipher[0] & 0xff18;//delta-rgt=0000 0000 ***0 1***


        if((tmp_rxdiff_cipher[0] == 0x0008) && (tmp_rxdiff_cipher[1] == 0x0010))// Collect the encrypted pairs after 12 rounds.
        {
                        ...
                        for(int i6 = 0; i6 < 2; i6++)
                        {
                            for(int i5 = 0; i5 < 2; i5++)
                            {
                                for(int i4 = 0; i4 < 2; i4++)
                                {
                                    ...
                                    rxdiff_guess_lft[0] = ROL(guess_lft[0],1) ^ guess_lft_pie[0];//Calculate the RX-difference

                                    rxdiff_guess_rgt[0] = ROL(guess_rgt[0],1) ^ guess_rgt_pie[0];


                                    rxdiff_guess_lft[1] = ROL(guess_lft[1],1) ^ guess_lft_pie[1];

                                    rxdiff_guess_rgt[1] = ROL(guess_rgt[1],1) ^ guess_rgt_pie[1];


                                    rxdiff_guess_lft[2] = ROL(guess_lft[2],1) ^ guess_lft_pie[2];

                                    rxdiff_guess_rgt[2] = ROL(guess_rgt[2],1) ^ guess_rgt_pie[2];





                                    if((rxdiff_guess_lft[2] == 0x0000) && (rxdiff_guess_rgt[2] == 0x0000)
                                        )
                                    {
